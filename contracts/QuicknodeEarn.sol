...
contract QuicknodeEarn is Ownable2Step, ReentrancyGuard {
    ...
    function withdrawAndBridge(
        ...
        // --- CCTP burn ---
        _cctpBurn(tokenMessenger, netUsdc, destDomain, mintRecipient, destinationCaller, maxFee, minFinalityThreshold);
    ...
    function selfBatchDeposit(
        ...
        for (uint256 i = 0; i < burns.length; i++) {
            _cctpBurn(burns[i].tokenMessenger, burns[i].amount, burns[i].destDomain,
    ...
    function selfBatchWithdraw(
        ...
        if (hasBurns) {
            ...
            for (uint256 i = 0; i < burns.length; i++) {
                ...
                _cctpBurn(burns[i].tokenMessenger, burnAmount, burns[i].destDomain,
                    burns[i].mintRecipient, burns[i].destinationCaller, burns[i].maxFee, burns[i].minFinalityThreshold);
