...
contract QuicknodeEarnProxy is Ownable2StepUpgradeable, ReentrancyGuardTransient, UUPSUpgradeable {
    ...
    ///         A constant lives in bytecode, not storage, so it is upgrade-safe.
    ...
    struct EarnStorage {
        mapping(address vault => bool approved) approvedVaults;
        address[] vaultList;
        address executor;
        address relayer;
    }
    ...
    function _validateBatchDeposit(
        ...
        for (uint256 i = 0; i < vaults.length; i++) {
            if (!_getEarnStorage().approvedVaults[vaults[i]]) revert VaultNotApproved(vaults[i]);
